...
public class TableListComparerTest {
  ...
  public void setUp() throws ParserException {
    HtmlTableScanner leftHandScanner = new HtmlTableScanner(
        "<table>empty</table>");
    HtmlTableScanner rightHandScanner = new HtmlTableScanner(
        "<table>empty</table>");
  ...
  @Test
  public void shouldOnlyReplaceAMatchIfThereIsNoBetterMatchForEitherTable()
      throws Exception {
    comparer.saveMatch(1, 1, 1.0);
  ...
  public void shouldRemoveOldMatchesIfBetterOnesAreFound() throws Exception {
    comparer.tableMatches.add(new MatchedPair(1, 1, 1.0));
    comparer.tableMatches.add(new MatchedPair(3, 2, 1.0));
    comparer.saveMatch(1, 2, 1.1);
    comparer.sortMatchesByScore();
    comparer.saveOnlyTheBestMatches();
  ...
  public void shouldGetAScoreBackFromCompareTables() throws Exception {
    ...
    assertEquals(MatchedPair.MAX_MATCH_SCORE, score, .01);
  }
  ...
  @Test
  public void shouldKeepTheBestScoreForATableEvenIfItIsHasABetterMatchItCantKeep()
      throws Exception {
    comparer.tableMatches.add(new MatchedPair(6, 6, 1.0));
  ...
  public void shouldBeAbleToOrderTheMatchesHighestToLowest() throws Exception {
    comparer.tableMatches.add(new MatchedPair(6, 6, 1.0));
    comparer.tableMatches.add(new MatchedPair(5, 5, .9));
    comparer.tableMatches.add(new MatchedPair(4, 4, 1.1));
    comparer.tableMatches.add(new MatchedPair(7, 7, 1.05));
    comparer.sortMatchesByScore();
    assertEquals(1.1, comparer.tableMatches.get(0).matchScore, .001);
